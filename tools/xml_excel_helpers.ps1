...
function New-ExcelNumberCell {
    ...
    [pscustomobject]@{
        __ExcelCell = $true
        Type        = 'Number'
        Value       = if ($null -eq $Value -or $Value -eq '') {
            ''
        }
        else {
...
function Get-ExcelXmlSheet {
    ...
    if ($Sheet.Kind -eq 'Object') {
        $headers = @()
        $firstRow = @($rows | Where-Object { $null -ne $_ } | Select-Object -First 1)
        if ($firstRow.Count -gt 0) {
            $headers = $firstRow[0].PSObject.Properties.Name
        }

        [void]$sb.AppendLine('<Row>')
        foreach ($header in $headers) {
            [void]$sb.AppendLine((Get-ExcelXmlCell (New-ExcelTextCell $header)))
        }
        [void]$sb.AppendLine('</Row>')
        ...
        foreach ($row in $rows) {
            [void]$sb.AppendLine('<Row>')
            foreach ($header in $headers) {
                [void]$sb.AppendLine((Get-ExcelXmlCell $row.$header))
            }
            [void]$sb.AppendLine('</Row>')
        }
    }
